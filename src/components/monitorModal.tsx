...
const HeartbeatCard: React.FC<{ item: any }> = ({ item }) => {
  return (
    <View className={`mb-2 rounded-lg border border-secondary p-4 shadow-md`}>
...
export const MonitorModal: React.FC<MonitorModalProps> = ({
  ...
  const transformChartData = React.useMemo(
    () =>
      latestHeartbeats.map((hb, index) => ({
        value: hb.ping,
        hideDataPoint: index % 4 !== 0, // Changed from 3 to 4 for better spacing
        customDataPoint: index % 4 === 0 ? ChartDataPoint : undefined,
        labelComponent:
          index % 12 === 0 // Changed from 9 to 12 for better spacing
            ? () => (
                <ChartLabel
  ...
  const chartConfig = React.useMemo(
    () => ({
      ...
      thickness: 2,
      color: '#1DB954',
      maxValue: Math.max(...latestHeartbeats.map((hb) => hb.ping)),
      noOfSections: 4,
      yAxisTextStyle: {
    ...
    >
      ...
      >
        ...
        <View className="flex-1 px-4 pt-12">
          ...
          <Text className="mb-2 text-lg font-bold">Latest Heartbeats</Text>
          <FlatList
